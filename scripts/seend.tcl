...
set isUsersOwn "Eu ainda não tinha visto voce! Tudo bem?"
#
## 2.When users supply an other nick and bot has NOT seen it yet:
set isOtherUser "Nao lembro ter visto este nick... Sera que ele ja falou algo em algum canal?"
#
## 3.When users supply the bots own name:
set isBotsName "A ultima vez que eu me vi foi... AQUI! Que coisa hein?"
#
# ----------------------------------------------------------
# Seen Results
# (0=disable,1=enable)
...
namespace eval seend {
   ...
   proc get_info {nick who} {
      ...
      if {$nick == $who} {
         set result "A ultima vez que te vi foi $last${chanmsg}.${storedmsg}"
      } else {
         set result "Vi o nick $who $last${chanmsg}.${storedmsg}"
      }
      return $result
   }
